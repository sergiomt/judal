...
public class RecordColumnValueComparatorAsc implements Comparator<Record> {
	...
	public RecordColumnValueComparatorAsc(String sColumnName, int columnType) {
		sColName = sColumnName;
		iColType = columnType;
		dtNull = null;
	}
	...
	public int compare(Record r1, Record r2) {
		...
		case Types.SMALLINT:
			if (r1.isNull(sColName) && r2.isNull(sColName))
				...
				return r1.getShort(sColName)>r2.getShort(sColName) ? 1 : r1.getShort(sColName)<r2.getShort(sColName) ? -1 : 0;	  		
		...
		case Types.FLOAT:
			if (r1.isNull(sColName) && r2.isNull(sColName))
				...
				return r1.getFloat(sColName)>r2.getFloat(sColName) ? 1 : r1.getFloat(sColName)<r2.getFloat(sColName) ? -1 : 0;
		...
		case Types.DOUBLE:
			if (r1.isNull(sColName) && r2.isNull(sColName))
				...
				return r1.getDouble(sColName)>r2.getDouble(sColName) ? 1 : r1.getDouble(sColName)<r2.getDouble(sColName) ? -1 : 0;
		...
		case Types.NUMERIC:
			if (r1.isNull(sColName) && r2.isNull(sColName))
				...
				return r1.getDecimal(sColName).compareTo(r2.getDecimal(sColName));
