...
public class DBJIterator implements AutoCloseable, Iterator<Stored> {
	...
	public DBJIterator(String sTbl, Database oPdb, StoredClassCatalog oCtg) {
		this.sTbl = sTbl;
		this.oPdb = oPdb;
		this.oCtg = oCtg;
		this.oCur = null;
		this.oNxt = new LinkedList<Stored>();
		oDbeb = new DBJEntityBinding(oCtg);
		open();
	}
		...
		if (null==oNext) {
			try {
				...
				if (oOst == OperationStatus.SUCCESS) {
					...
					oNext = new DBJStored(new TableDef(sTbl));
