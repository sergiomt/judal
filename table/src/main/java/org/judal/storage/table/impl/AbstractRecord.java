...
public abstract class AbstractRecord extends AbstractRecordBase {
	 ...
	 */
	@Override
	public Object getMap(String sColName) throws ClassCastException, InstantiationException, IllegalAccessException, IllegalArgumentException, NoSuchMethodException, SecurityException, ClassNotFoundException {
		if (null==fhelper)
			throw new ClassNotFoundException("No FieldHelper class has been specified for " + getTableName());
			...
			if (null==aInts) {
				...
				if (bIsSerializable) {
					try {
						...
						if (aBytes!=null) {
							if (!hasLongVarBinaryData) longVarBinariesLengths = new HashMap<>();
							longVarBinariesLengths.put(sKey, (long) aBytes.length);
							hasLongVarBinaryData = true;
							retObj = aBytes;
						}
						oOOut.close();
