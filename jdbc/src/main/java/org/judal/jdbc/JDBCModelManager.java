...
import static org.judal.jdbc.metadata.SQLColumn.Blob;
import static org.judal.jdbc.metadata.SQLColumn.Clob;
import static org.judal.jdbc.metadata.SQLColumn.CurrentTimeStamp;
import static org.judal.jdbc.metadata.SQLColumn.LongVarBinary;
import static org.judal.jdbc.metadata.SQLColumn.LongVarChar;
import static org.judal.jdbc.metadata.SQLColumn.Numeric;
import static org.judal.jdbc.metadata.SQLColumn.Serial;
import static org.judal.jdbc.metadata.SQLColumn.Timestamp;
import static org.judal.jdbc.metadata.SQLColumn.VarChar;
...
public abstract class JDBCModelManager {
	...
	public JDBCModelManager() {

		if (DebugFile.trace) {
			DebugFile.writeln("ModelManager build " + VERSION);
			DebugFile.envinfo();
		}
	...
	public void connect(String sDriver, String sUrl, String sSch, String sUsr, String sPwd)
		...
		if (DebugFile.trace) {
			DebugFile.writeln("Begin DBModelManager.connect(" + sDriver + "," + sUrl + ", ...)");
			DebugFile.incIdent();
		}
		...
		if (DebugFile.trace) {
			DebugFile.decIdent();
			DebugFile.writeln("End DBModelManager.connect()");
		}
	...
	public void disconnect() throws SQLException {
		...
		if (DebugFile.trace) {
			DebugFile.writeln("Begin DBModelManager.disconnect()");
			DebugFile.incIdent();
		}
		...
		if (DebugFile.trace) {
			DebugFile.decIdent();
			DebugFile.writeln("End DBModelManager.disconnect()");
		}
	} // disconnect
	...
	public void setConnection(Connection oJDBCConn)
		...
		if (DebugFile.trace) {
			DebugFile.writeln("Begin DBModelManager.setConnection([Connection])");
			DebugFile.incIdent();
		}

		if (null==oJDBCConn) throw new NullPointerException("Connection parameter may not be null");

		oConn = new JDCConnection(oJDBCConn, null);
	 ...
	 */
	public static String translate(String sSQL, int iDbms) throws NullPointerException, UnsupportedOperationException {

		String sRetSql;

		if (iDbms>=VarChar.length)
			throw new UnsupportedOperationException("ModelManager.translate() SQL Translation is not implemented for RDBMS Id "+String.valueOf(iDbms));

			if (DebugFile.trace) {
			DebugFile.writeln("Begin ModelManager.translate(" + sSQL + ") for "+String.valueOf(iDbms));
			DebugFile.incIdent();
		}
		...
		if (DebugFile.trace) {
			DebugFile.decIdent();
			DebugFile.writeln("End DBModelManager.translate() :\n" + sRetSql + "\n");
		}
		return sRetSql;
	} // translate
	...
	private String[] split(StringBuffer oBuffer, char cDelimiter, String sGo) {
		...
		for (int c=0; c<iLen; c++) {
			...
			if (iGo>0 && RDBMS.POSTGRESQL.intValue()==iDbms) {
	...
	public void executeSQLScript (String sScriptSource, String sDelimiter)
		...
		if (DebugFile.trace) {
			DebugFile.writeln("Begin DBModelManager.executeSQLScript(String, "+sDelimiter+")");
			DebugFile.incIdent();
		}
		...
		if (DebugFile.trace) {
			DebugFile.decIdent();
			DebugFile.writeln("End DBModelManager.executeSQLScript()");
		}
	}
	...
	private StringBuffer getSQLBuffer(String sResourcePath, int iBatchType)
		...
		if (DebugFile.trace) {
			DebugFile.writeln("Begin DBModelManager.getSQLBuffer(" + sResourcePath + ")");
			DebugFile.incIdent();
		}
		...
		if (DebugFile.trace) {
			DebugFile.decIdent();
			DebugFile.writeln("End DBModelManager.getSQLBuffer() : " + String.valueOf(oBuffer.length()));
		}
	...
	protected int executeBulk(StringBuffer oBuffer, String sResourcePath, int iBatchType)
		...
		if (DebugFile.trace) {
			DebugFile.writeln("Begin DBModelManager.executeBulk(" + sResourcePath + "," +
		...
		case BULK_BATCH:
			...
			for (int s = 0; s < iStatements; s++) {
				...
				try {
					sSQL = translate(aStatements[s], iDbms);
				}
	...
	public String getResourceAsString (String sResourcePath, String sEncoding)
		...
		if (DebugFile.trace) {
			DebugFile.writeln("Begin DBModelManager.getResourceAsString(" + sResourcePath + "," + sEncoding + ")");
			DebugFile.incIdent();
		}
		...
		if (DebugFile.trace) {
			DebugFile.decIdent();
			DebugFile.writeln("End DBModelManager.getResourceAsString()");
		}
	...
	protected LinkedList listConstraints (Connection oJCon)
			throws SQLException {

		if (DebugFile.trace) {
			DebugFile.writeln("Begin DBModelManager.listConstraints()");
			DebugFile.incIdent();
		}

		LinkedList<Constraint> oConstraintList = new LinkedList<Constraint>();
